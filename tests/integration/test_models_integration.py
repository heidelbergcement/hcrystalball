...
@pytest.mark.parametrize('X_y_linear_trend, wrapper_instance,horizon',[
...
def test_model_fit_predict(X_y_linear_trend, wrapper_instance, horizon):
    ...
    expected_result = pd.DataFrame(y, 
                                   index = X.index, 
                                   columns = [wrapper_instance.name])
...
def pipeline(request):
    ...
    if 'with_model' in request.param:
        return Pipeline([
            ('preproc', TSColumnTransformer(transformers=[
                ('raw_cols_1', 'passthrough', ['trend']),        
    ...
    if 'more_layers_builtin_transformers' in request.param:
        return Pipeline([
            ('first', TSColumnTransformer(transformers=[
                ('raw_cols_1', 'passthrough', ['trend']),
                ('one_hot', OneHotEncoder(sparse=False), ['one_hot']),
                ('scaler', StandardScaler(), ['trend']),
            ])),
            ('second', TSColumnTransformer(transformers=[
...
@pytest.mark.parametrize('X_y_linear_trend, pipeline, exp_cols',[
    ('more_cols_freq_D','passthrough_position',['one_hot','trend']),
    ('more_cols_freq_D','col_name_clash',['trend','trend_scaler']),  
    ('more_cols_freq_D','more_dimensions_with_get_feature_names',['trend','x0_1', 'x0_2', 'x0_3', 'x0_4']),
    ...
    ('more_cols_freq_D','more_layers_custom_transformers_same_level_country_code',['x0_', 'x0_New year', 'trend']),
    ('more_cols_country_col_freq_D','more_layers_custom_transformers_same_level_country_code_country_col',['x0_', 'x0_New year', 'trend']),
    ('more_cols_country_col_freq_D','more_layers_holiday_in_column_transformer',['x0_', 'x0_New year', 'trend', 'country']),
