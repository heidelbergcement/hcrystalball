    ...
    if request.param == 'prophet':
        ...
        return StackingEnsemble(
            base_learners=[
                ...
                ], meta_model=LinearRegression(),horizons_as_features=False, weekdays_as_features=False)
...
def wrapper_instance_capped(request):
    if request.param.split(';')[0] == 'prophet':
        ...
        return get_sklearn_wrapper(LinearRegression,
                                   lags=4,
                                   clip_predictions_lower=float(request.param.split(';')[1]),
                                   clip_predictions_upper=float(request.param.split(';')[2]))
        ...
        return StackingEnsemble(
            base_learners=[
                ...
                ExponentialSmoothingWrapper(name='smoot_exp2',
                                            clip_predictions_lower=float(request.param.split(';')[1]),
                                            clip_predictions_upper=float(request.param.split(';')[2])),
                ], meta_model=LinearRegression(),horizons_as_features=False, weekdays_as_features=False, train_n_splits=1, train_horizon=10, 
                clip_predictions_lower=float(request.param.split(';')[1]),
                clip_predictions_upper=float(request.param.split(';')[2]))
...
def X_y_linear_trend(request):
    ...
    X = pd.DataFrame(pd.date_range(start='2019-01-01', periods=100, freq=request.param.split('freq_')[1][0]), columns=['date'])
...
def pipeline_instance_model_only(request):
    ...
    if request.param == 'prophet':
        return Pipeline([('regressor', ProphetWrapper(daily_seasonality=False,
                                                      weekly_seasonality=False,
                                                      yearly_seasonality=False))])
    elif request.param == 'smoothing':
        ...
        return Pipeline([('regressor', StackingEnsemble(base_learners=[
            ExponentialSmoothingWrapper(name='smoot_exp1', trend='add'),
            ExponentialSmoothingWrapper(name='smoot_exp2')],
            meta_model=LinearRegression()))])

    elif request.param == 'simple_ensemble':
        return Pipeline([('regressor', SimpleEnsemble(base_learners=[
            ExponentialSmoothingWrapper(name='smoot_exp1', trend='add'),
            ExponentialSmoothingWrapper(name='smoot_exp2')]))])
    else:
        return None
...
def pipeline_instance_model_in_pipeline(request):
    ...
    if request.param == 'prophet':
        return Pipeline([('model', Pipeline([('regressor', ProphetWrapper(daily_seasonality=False,
                                                                          weekly_seasonality=False,
                                                                          yearly_seasonality=False))])
                          )])
        ...
        return Pipeline([('model', Pipeline([('regressor', TBATSWrapper(use_arma_errors=False, use_box_cox=False))])
                          )])

    elif request.param == 'sklearn':
        return Pipeline([('model', Pipeline([('regressor', get_sklearn_wrapper(LinearRegression, lags=4))])
                          )])

    elif request.param == 'sarimax':
        return Pipeline([('model', Pipeline([('regressor', SarimaxWrapper(order=(1, 1, 0),
                                                                          seasonal_order=(1, 1, 1, 1)))])
                          )])

    elif request.param == 'stacking_ensemble':
        return Pipeline([('model', Pipeline([('regressor', StackingEnsemble(base_learners=[
            ExponentialSmoothingWrapper(name='smoot_exp1', trend='add'),
            ExponentialSmoothingWrapper(name='smoot_exp2')],
            meta_model=LinearRegression()))])
                          )])
        ...
        return Pipeline([('model', Pipeline([('regressor', SimpleEnsemble(base_learners=[
            ExponentialSmoothingWrapper(name='smoot_exp1', trend='add'),
            ExponentialSmoothingWrapper(name='smoot_exp2')]))])
                          )])
    else:
...
def test_data_raw():
    ...
    for region in regions:
        df_tmp = pd.DataFrame(columns=['date','Region', 'Plant', 'Product', 'Quantity'], index=range(len(dates)))        
...
def train_data(request):
    ...
    dfs = []
    df_tmp = pd.DataFrame(columns=['date','Region', 'Product', 'Holidays_code', 'Quantity'], index=range(len(df0.index)))
    for region in regions:
        df_tmp.loc[:, 'Region'] = region
        for product in products:
...
def grid_search(request):
    ...
    bad_dummy = get_sklearn_wrapper(DummyRegressor, strategy="constant", constant=42, name="bad_dummy", lags=2)
    good_dummy = get_sklearn_wrapper(DummyRegressor, strategy="mean", name="good_dummy", lags=2)

    parameters = [{'model': [good_dummy]},
                  {'model': [bad_dummy],
                   'model__strategy': ["constant"],
                   'model__constant': [42]}]
    
    holiday_model = Pipeline([('holiday', HolidayTransformer(country_code_column='Holidays_code')), 
...
@pytest.fixture()
def unprepared_data():
    increment = pd.DataFrame({'date': ['2019-05-01',
                                       '2019-05-01',
                                       '2019-05-01',
                                       '2019-05-02'],
                              'cem_type': ['CEM 52,5 R-ft',
                                           'CEM 52,5 R-ft',
                                           'CEM 52,5 N',
                                           'CEM 52,5 R'
                                           ],
...
def prepared_data(request):
    if 'without_logical_partition' in request.param:
        ...
        increment = pd.DataFrame({'date': ['2019-05-01', '2019-05-02'],
                    'target': [75.0, 5.0],
                    'cem_type': ['CEM 52,5 N', 'CEM 52,5 R'],
                    'dummy': ['dummy', 'data']})
        
    else:
        increment = pd.DataFrame({'date': ['2019-05-01', '2019-05-02', '2019-05-01'],
                                'cem_type': ['CEM 52,5 N',
                                            'CEM 52,5 R',
                                            'CEM 52,5 R-ft'
                                            ],
