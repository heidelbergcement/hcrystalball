...
@pytest.mark.parametrize(
    "train_data, grid_search", [("", "")], indirect=["train_data", "grid_search"]
)
def test_persist_experts_in_physical_partition(train_data, grid_search, tmp_path):
    ...
    for result in results:
        with open(
            os.path.join(tmp_path, result.partition_hash + ".cv_results"), "rb"
        ) as file:
        ...
        with open(
            os.path.join(tmp_path, result.partition_hash + ".cv_data"), "rb"
        ) as file:
        ...
        with open(
            os.path.join(tmp_path, result.partition_hash + ".model_reprs")
        ) as file:
        ...
        with open(
            os.path.join(tmp_path, result.partition_hash + ".best_model"), "rb"
        ) as file:
...
def test_load_expert(train_data, grid_search, tmp_path):
    ...
    for result in results:
        cv_results = _load_file(
            partition_label=result.partition, path=tmp_path, expert_type="cv_results"
        )
        assert isinstance(cv_results, type(result.cv_results))
        assert all(cv_results.columns == result.cv_results.columns)

        cv_data = _load_file(
            partition_label=result.partition, path=tmp_path, expert_type="cv_data"
        )
        assert isinstance(cv_data, type(result.cv_data))
        assert all(cv_data.columns == result.cv_data.columns)

        model_reprs = _load_file(
            partition_label=result.partition, path=tmp_path, expert_type="model_reprs"
        )
        assert isinstance(model_reprs, type(result.model_reprs))
        assert model_reprs == result.model_reprs

        pkl_model = _load_file(
            partition_label=result.partition, path=tmp_path, expert_type="best_model"
        )
        assert isinstance(pkl_model, type(result.best_model))
        assert str(pkl_model.get_params()) == str(result.best_model.get_params())

        partition = _load_file(
            partition_label=result.partition, path=tmp_path, expert_type="partition"
        )
